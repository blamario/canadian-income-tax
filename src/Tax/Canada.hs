...
completeRelevantForms :: Province.Code -> Federal.InputForms Maybe -> FDFs -> Either String FDFs
...
filterRelevant :: T1 Maybe -> FDFs -> FDFs
filterRelevant t1 = flip Map.restrictKeys (relevantFormKeys t1 <> alwaysRelevant)
  where alwaysRelevant = Set.fromDistinctAscList ["428", "479", "T1"]

-- | Like 'mapForms', but also returns the T1 form by itself.
mapFormsWithT1 :: (Rank2.Apply form, Rank2.Traversable form)
               => form FDF.FieldConst -> (form Maybe -> T1 Maybe) -> (form Maybe -> form Maybe) -> FDFs
               -> Either String (T1 Maybe, FDFs)
...
formFileNames p = Map.fromList [("T1", T1.fileNameForProvince p)] <> Federal.formFileNames
